...
class AccountInvoice(models.Model):
    ...
    @api.depends('tax_line_ids', 'state', 'move_type')
                ...
                if inv.move_type == "in_invoice":
                    ...
                    inv.withholded_itbis = abs(
                        inv._convert_to_local_currency(
                            sum(
                                tax_line_ids.filtered(lambda tax: tax.tax_id.purchase_tax_type == "ritbis").mapped(
                                    "amount"
                                )
                            )
                        )
                    ...
                    inv.income_withholding = abs(
                        inv._convert_to_local_currency(
                            sum(
                                tax_line_ids.filtered(lambda tax: tax.tax_id.purchase_tax_type == "isr").mapped(
                                    "amount"
                                )
                            )
                        )
                ...
                for payment in inv._get_invoice_payment_widget():
                    ...
                    if inv.move_type == "out_invoice":
                        ...
                        inv.third_withheld_itbis += sum(
                            self._get_payment_move_iterator(payment, inv.move_type, witheld_itbis_types)
                        )

                        # Retención de Renta pr Terceros
                        inv.third_income_withholding += sum(
                            self._get_payment_move_iterator(payment, inv.move_type, witheld_isr_types)
                        )
    ...
    isr_withholding_type = fields.Char(compute="_compute_isr_withholding_type", store=False, size=2)
    ...
    @api.depends("tax_line_ids", "payment_state", "state")
        ...
        for inv in self:
            ...
            if inv.state != "draft":
                ...
                inv.selective_tax = inv._convert_to_local_currency(
                    sum(tax_line_ids.filtered(lambda tax: tax.tax_id.tax_group_id.name == "ISC").mapped("amount"))
                )

                # Monto Otros Impuestos/Tasas
                inv.other_taxes = inv._convert_to_local_currency(
                    sum(
                        tax_line_ids.filtered(lambda tax: tax.tax_id.tax_group_id.name == "Otros Impuestos").mapped(
                            "amount"
                        )
                    )
                )
                ...
                inv.legal_tip = inv._convert_to_local_currency(
                    sum(tax_line_ids.filtered(lambda tax: tax.tax_id.tax_group_id.name == "Propina").mapped("amount"))
                )

                # ITBIS sujeto a proporcionalidad
                inv.proportionality_tax = inv._convert_to_local_currency(
                    sum(
                        tax_line_ids.filtered(lambda tax: tax.account_id.account_fiscal_type in ["A29", "A30"]).mapped(
                            "amount"
                        )
                    )
                )
                ...
                inv.cost_itbis = inv._convert_to_local_currency(
                    sum(tax_line_ids.filtered(lambda tax: tax.account_id.account_fiscal_type == "A51").mapped("amount"))
                )
