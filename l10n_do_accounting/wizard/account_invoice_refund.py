...
class AccountInvoiceRefund(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        res["is_fiscal_refund"] = set(invoice_ids.mapped("is_l10n_do_fiscal_invoice")) == {True}
    ...
    refund_type = fields.Selection(selection=_get_refund_type_selection, default=_get_default_refund_type,)
    refund_method = fields.Selection(
        selection=_get_refund_method_selection, default="draft_refund", string="Refund Method",
    )
    percentage = fields.Float()
    amount = fields.Float()
    account_id = fields.Many2one(
        "account.account", string="Account", domain=[("deprecated", "=", False)], default=_default_account,
    )
    ...
    def compute_refund(self, mode="refund"):
        ...
        for wizard in self:
            if wizard.refund_type == "full_refund":
                return super(
                    AccountInvoiceRefund, self.with_context(refund_ref=wizard.refund_ref),
            ...
            for inv in inv_obj.browse(context.get("active_ids")):
                ...
                amount = (
                    wizard.amount if refund_type == "fixed_amount" else inv.amount_untaxed * (wizard.percentage / 100)
                )
        ...
        for wizard in self:
            ...
            for inv in inv_obj.browse(context.get("active_ids")):
                ...
                fiscal_type = self.env["account.fiscal.type"].search(
                    [("type", "=", context.get("debit_note")), ("state", "=", "active")]
                )
                ...
                values = {
                    ...
                    "invoice_line_ids": [
                        (0, 0, {"name": description, "account_id": wizard.account_id.id, "price_unit": amount,},)
                    ],
        ...
        if active_id:
            ...
            if self.refund_ref and self.is_fiscal_refund:
                ...
                if self._context.get("debit_note") and ncf not in ("B03", "E33"):
                    raise UserError(_("Debit Notes must be type B03 or E33, this NCF " "structure does not comply."))
                elif ncf not in ("B04", "E34"):
                    raise UserError(
                        _(
                            ("Credit Notes must be type B04 or E34, this NCF (Type %s)" " structure does not comply.")
                            % ncf
                        )
                    )
                elif (ncf_digits != 11 and ncf == "B04") or (ncf_digits != 13 and ncf == "E34"):
                    raise UserError(
                        _(
                            (
                ...
                # TODO move this to l10n_do_external_validation_ncf
                elif invoice.journal_id.l10n_do_ncf_remote_validation and not ncf_validation.check_dgii(
                    invoice.partner_id.vat, self.refund_ref
                ):
                    raise ValidationError(
                        _(
                            ...
                            "yet."
                        ).format(self.refund_ref, self.partner_id.name)
                    )

        return super(AccountInvoiceRefund, self).reverse_moves()
